...
public class ServiceLinePaymentRequestMapper {
    ...
    public ServiceLinePaymentRequest map(ERADataTransferModel era, List<ERALineTransferModel> historyLines) {
    ...
    private void createPayments(ServiceLinePaymentRequest serviceLinePaymentRequest, List<ERALineTransferModel> historyLines) {
        List<SessionServiceLinePayment> serviceLinePayments = new ArrayList<>();
        getSessionMap(historyLines.stream().map(eraLineHistory -> new Long(eraLineHistory.getServiceLineID()))
                .collect(Collectors.toList()));
        for (int i = 0; i < historyLines.size(); i++) {
            ERALineTransferModel line = historyLines.get(i);
            ...
            SessionServiceLinePayment payment = new SessionServiceLinePayment(balance, line.getPaidAmount()
                    , line.getAdjustAmount(), new Long(line.getServiceLineID()), new Date().getTime(), ServiceLinePaymentType.InsuranceCompany);
            payment.setServiceLinePaymentAction(ServiceLinePaymentAction.valueOf(line.getAction()));
            payment.setServiceLineId(new Long(line.getServiceLineID()));
            payment.setSessionId(sessionToServiceLineMapper.get(new Long(line.getServiceLineID())));
