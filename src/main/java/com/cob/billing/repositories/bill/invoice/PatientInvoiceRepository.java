...
public interface PatientInvoiceRepository extends PagingAndSortingRepository<PatientInvoiceEntity, Long> {
    ...
    @Query("SELECT DISTINCT pi FROM PatientInvoiceEntity pi INNER JOIN FETCH  pi.invoiceDetails pid " +
            ...
            "AND ((:provider is null or upper(FUNCTION('json_extract_path_text','pid.patientSession.doctorInfo, '$.doctorFirstName')) LIKE CONCAT('%',:provider,'%') OR (:provider is null or upper(FUNCTION('json_extract_path_text',pid.patientSession.doctorInfo, '$.doctorLastName')) LIKE CONCAT('%',:provider,'%'))))" +
            "AND (:submitStart is null or pi.createdAt >= :submitStart) " +
            "AND (:submitEnd is null or pi.createdAt <= :submitEnd)  " +
            "AND (:insuranceCompany is null or upper(FUNCTION('json_extract_path_text',pi.insuranceCompany, '$.name')) LIKE CONCAT('%',:insuranceCompany,'%')) " +
