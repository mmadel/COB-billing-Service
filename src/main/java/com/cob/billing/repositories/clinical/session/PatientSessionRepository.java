...
public interface PatientSessionRepository extends PagingAndSortingRepository<PatientSessionEntity, Long> {
    ...
    @Query("SELECT  ps FROM PatientSessionEntity ps  JOIN ps.serviceCodes serviceCode WHERE serviceCode.id  IN :serviceCodeIds")
    List<PatientSessionEntity> findSessionByServiceCodeIds(@Param("serviceCodeIds") List<Long> ids);
    ...
    @Query("SELECT DISTINCT s FROM PatientSessionEntity s INNER JOIN FETCH s.serviceCodes sc " +
            ...
            "AND ((:provider is null or upper(FUNCTION('jsonb_extract_path_text',s.doctorInfo, '$.doctorFirstName')) LIKE CONCAT('%',:provider,'%')) " +
            "OR ((:provider is null or upper(FUNCTION('jsonb_extract_path_text',s.doctorInfo, '$.doctorLastName')) LIKE CONCAT('%',:provider,'%'))) ) ")
    ...
    @Query("SELECT  DISTINCT s FROM PatientSessionEntity s INNER JOIN FETCH  s.serviceCodes sc " +
            "WHERE s.patient.id= :patientId AND (s.status = 'Submit' OR s.status = 'Partial') AND sc.type  IN ('Invoice') " +
            "AND (:dateFrom is null or s.serviceDate >= :dateFrom) " +
            "AND (:dateTo is null or s.serviceDate <= :dateTo)")
